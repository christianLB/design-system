...
const characterSets = {
  matrix: ['ｱ', 'ｲ', 'ｳ', 'ｴ', 'ｵ', 'ｶ', 'ｷ', 'ｸ', 'ｹ', 'ｺ', 'ｻ', 'ｼ', 'ｽ', 'ｾ', 'ｿ', 'ﾀ', 'ﾁ', 'ﾂ', 'ﾃ', 'ﾄ', 'ﾅ', 'ﾆ', 'ﾇ', 'ﾈ', 'ﾉ', 'ﾊ', 'ﾋ', 'ﾌ', 'ﾍ', 'ﾎ', 'ﾏ', 'ﾐ', 'ﾑ', 'ﾒ', 'ﾓ', 'ﾔ', 'ﾕ', 'ﾖ', 'ﾗ', 'ﾘ', 'ﾙ', 'ﾚ', 'ﾛ', 'ﾜ', 'ｦ', 'ﾝ'],
  binary: ['0', '1'],
    ...
    afterThemeBuild: (context: PluginContext): PluginResult => {
      ...
      const utilityClasses: Record<string, Record<string, any>> = {
        ...
        '.cyber-matrix-overlay': {
          position: 'relative',
          '&::after': {
            content: '"' + characterSets[config.characters || 'matrix'].slice(0, 20).join(' ') + '"',
            position: 'absolute',
            top: '0',
            left: '0',
            right: '0',
...
export const createCyberpunkMatrixPlugin = (config: Partial<CyberpunkMatrixConfig> = {}): AnimationPlugin => ({
  ...cyberpunkMatrixPlugin,
  config: { ...defaultConfig, ...config },
});

// Helper function to generate matrix characters for CSS content
export const generateMatrixCharacters = (charset: keyof typeof characterSets, count: number = 20): string => {
